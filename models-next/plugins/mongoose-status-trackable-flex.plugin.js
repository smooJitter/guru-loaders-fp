...
import * as STATUS_CONSTANTS from '../config/status.constants.js';

function statusTrackable(schema, options = {}) {
  const {
    embedded = true,
  ...
  const applicableStatuses = allowedTypes
    ? STATUS_CONSTANTS.STATUS_ENUMS.filter(
        (key) =>
          STATUS_CONSTANTS.STATUS_META[key]?.type === allowedTypes ||
          (Array.isArray(allowedTypes) && allowedTypes.includes(STATUS_CONSTANTS.STATUS_META[key]?.type))
      )
    : STATUS_CONSTANTS.STATUS_ENUMS;
  ...
  if (embedded) {
    ...
    schema.methods.setStatus = function (newStatus, updatedBy = null, reason = null) {
      ...
      const currentMeta = STATUS_CONSTANTS.STATUS_META[currentStatus];
...
export { statusTrackable };
export default statusTrackable;
