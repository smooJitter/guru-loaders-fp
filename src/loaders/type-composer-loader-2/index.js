...
const getFieldTypeConfig = (fieldName, field, logger) => {
  try {
    if (!field) {
      logger.warn(`[type-composer-loader-2] Invalid field definition for ${fieldName}`);
      return { type: 'String' };
    }
    ...
    logger.error(`[type-composer-loader-2] Error getting field type for ${fieldName}:`, err);
    return { type: 'String' };
  }
};
...
const validateModel = (modelName, model, logger) => {
  try {
    ...
    logger.warn(`[type-composer-loader-2] Invalid model: ${modelName}`);
    return null;
  } catch (err) {
    logger.error(`[type-composer-loader-2] Error validating model ${modelName}:`, err);
    return null;
  }
};
...
const transformTypeComposer = async (modules, ctx) => {
  ...
  if (!ctx || typeof ctx !== 'object') {
    throw new Error('[type-composer-loader-2] Invalid context provided');
  }
  ...
  if (ctx.models && typeof ctx.models === 'object') {
    if (Object.keys(ctx.models).length === 0) {
      throw new Error('[type-composer-loader-2] No models provided');
    }
    ...
    for (const [modelName, model] of Object.entries(ctx.models)) {
      try {
        ...
        if (ctx.typeComposers[modelName]) {
          logger.warn(`[type-composer-loader-2] Duplicate typeComposer name: ${modelName}`);
        }

        ctx.typeComposers[modelName] = typeComposer;
      } catch (err) {
        logger.error(`[type-composer-loader-2] Error creating typeComposer for ${modelName}:`, err);
  ...
  if (ctx.plugins) {
    if (!Array.isArray(ctx.plugins)) {
      throw new Error('[type-composer-loader-2] Plugins must be an array');
    }

    for (const plugin of ctx.plugins) {
      if (typeof plugin !== 'function') {
        logger.warn('[type-composer-loader-2] Invalid plugin: must be a function');
        continue;
      }
      ...
      try {
        ...
        logger.error('[type-composer-loader-2] Plugin error:', err);
