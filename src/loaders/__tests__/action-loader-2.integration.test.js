import { createActionLoader2, actionLoader2 } from '../action-loader-2/index.js';
import { withNamespace } from 'guru-loaders-fp/utils';
import { extractActions } from '../action-loader-2/extractActions.js';
...
describe('action-loader-2 index.js edge cases', () => {
  ...
  it('returns empty actions if no findFiles/importModule', async () => {
    const result = await actionLoader2({ ...context, options: {} });
  ...
  it('uses custom patterns if provided', async () => {
    ...
    await actionLoader2({ ...context, options });
  ...
  it('handles error in errorHandlingHook', async () => {
    ...
    // await expect(actionLoader2({ ...context, options: {} })).rejects.toThrow('fail');
    // spy.mockRestore();
