// Utility to flatten and aggregate actions from modules for action-loader-2
  ...
  for (const mod of modules) {
    ...
    for (const action of actionsArr) {
      ...
      if (registry[action.namespace][action.name]) {
        context?.services?.logger?.warn?.(`[action-loader-2] Duplicate action: ${action.namespace}.${action.name}`);
      }
